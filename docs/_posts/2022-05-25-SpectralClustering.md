...
- An (undirected) **graph** is $$G=(V,E)$$, where
    ...
    - $$E=\{(v_i,v_j)\vert v_i,v_j\in V\}$$ is a subset of $$V\times V$$
...
- degree: for any $$v_i\in V$$, the **degree** $$d(v_i)$$ of $$v_i$$ is the number of edges adjacent to  $$v_i$$: $$d(v_i)=\vert \{v_j\in V\vert \{v_j,v_i\}\in E\vert $$
...
- Given $$G=(V,E)$$, with $$\vert V\vert =n$$ and $$\vert E\vert =m$$, the **adjacency matrix** $$A=(a_{ij})$$ of $$G$$ is **symmetric** $$n\times n$$ matrix with
...
- where ***D*** is the degree matrix of ***G***, i.e., $$D=diag(W\cdot \vec 1)$$
...
- $$L\cdot \vec 1=\vec 0$$
    - Proof: $$L\cdot \vec 1=(D-W)\cdot\vec 1=D\cdot\vec 1-W\cdot\vec 1=\vec d-\vec d=\vec 0$$
- If the underlying graph of ***G*** is connected
    ...
    - The smallest eigenvalue of ***L*** is 0, the corresponding eigenvector is the constant one vector $$\vec 1$$.
    - the dimension of the nullspace of ***L*** is **1.**
- The multiplicity ***k*** of the eigenvalue ***0*** of L equals **the number of connected components** $$A_1,\dots,A_k$$ in the graph. The eigenspace of eigenvalue 0 is spanned by the indicator vectors $$\vec 1_{A_1},\dots,\vec 1_{A_k}$$ of those components.

### Normalized Graph Laplacian

- $$L_{sym}:= D^{-1/2}LD^{1/2}=I-D^{-1/2}WD^{1/2}$$
    - Denote as $$L_{sym}$$ as it’s a symmetric matrix
- $$L_{rm}:= D^{-1/2}L=I-D^{-1}W$$
...
- 0 is an eigenvalue of $$L_{rw}$$ with the constant one vector $$\vec 1$$ as eigenvector. 0 is an eigenvalue of $$L_{sym}$$ with eigenvector $$D^{1/2}\vec 1$$.
...
- The multiplicity ***k*** of the eigenvalue ***0*** of both $$L_{sym}$$ and $$L_{rw}$$ equals **the number of connected components** $$A_1,\dots,A_k$$ in the graph. For $$L_{rw}$$, the eigenspace of eigenvalue 0 is spanned by the indicator vectors $$\vec 1_{A_i}$$ of those components. For $$L_{sym}$$, the eigenspace of eigenvalue 0 is spanned by the vectors $$D^{1/2}\vec 1_{A_i}$$.
...
$$
\min_{A_1,\dots,A_k}RatioCut(A_1,\dots,A_k):=\min_{A_1,\dots,A_k}\frac{1}{2}\sum_{i=1}^k\frac{W(A_i,\bar A_i)}{\vert A_i\vert }
$$

- The minimum of $$\sum_{i=1}^k\frac{1}{\vert A_i\vert }$$ is achieved if all $$\vert A_i\vert $$ coincide
...
- Rewrite the objective function
    - Define the vector $$f=(f_1,\dots,f_n)'\in\mathbb{R}^n$$ with entries $$f_i=\begin{cases}\sqrt{\vert \bar A\vert /\vert A\vert },\text{ if } v_i\in A\\ -\sqrt{\vert A\vert /\vert \bar A\vert },\text{ if } v_i\in \bar A\end{cases}$$
    - It can be proven that $$f'Lf=\vert V\vert \cdot RatioCut(A,\bar A)$$
    - Additionally, we have $$\sum_{i=1}^n f_i=\sum_{i\in A}\sqrt{\frac{\vert \bar A\vert }{\vert A\vert }}-\sum_{i\in \bar A}\sqrt{\frac{\vert A\vert }{\vert \bar A\vert }}=\vert A\vert \sqrt{\frac{\vert \bar A\vert }{\vert A\vert }}-\vert \bar A\vert \sqrt{\frac{\vert A\vert }{\vert \bar A\vert }}=0$$
        - In other words, $$f$$ is orthognal to the constant one vector $$\vec 1$$
    - Finally, note that $$f$$ satisfies $$\| f\| ^2=\sum_{i=1}^nf_i^2=\vert \bar A\vert +\vert A\vert =n$$
    - Put them together, we have
        
        $$
        \min_{A\subset V}f'Lf \text{ subject to } f\perp\vec 1,f_i=\begin{cases}\sqrt{\vert \bar A\vert /\vert A\vert }\text{ if } v_i\in A\\ -\sqrt{\vert A\vert /\vert \bar A\vert }\text{ if } v_i\in \bar A\end{cases},\| f\| =\sqrt n
        $$
...
- Leads to the relaxed optimization problem
    ...
    $$
    \min_{f\in\mathbb{R}^n}f'Lf \text{ subject to } f\perp\vec 1,\| f\| =\sqrt n
    $$
...
- By the Rayleigh-Ritz theorem, the solution is given by the vector $$f$$ which is the eigenvector corresponding to the second smallest eigenvalue of $$L$$
    - recall that the smallest eigenvalue of $$L$$ is 0 with eigenvector $$\vec 1$$
...
- Given a partition of ***V*** into k sets $$A_1,\dots,A_k$$, we define k indicator vectors $$h_j=(h_{1,j},\dots,h_{n,j})'$$ by $$h_{i,j}=\begin{cases}1/\sqrt{\vert A_j\vert },\text{ if } v_i\in A_j\\ 0,\text{ otherwise}\end{cases}\quad (i=1,\dots,n;\;j=1,\dots,k)$$
...
- Rewrite the objective function
    - Similarly, we have $$h_i'Lh_i=\frac{cut(A_i,\bar A_i)}{\vert A_i\vert }$$.
    ...
    $$
    \min_{A_1,\dots,A_k}Tr(H'LH)\text{ subject to }H'H=I,h_{i,j}=\begin{cases}1/\sqrt{\vert A_j\vert },\text{ if } v_i\in A_j\\ 0,\text{ otherwise}\end{cases}
    $$
...
- Rewrite the objective function
    ...
    - Additionally, we have $$(Df)'\vec 1=0$$
        - In other words, $$Df$$ is orthognal to the constant one vector $$\vec 1$$
    - Finally, note that $$f'Df=vol(V)$$
    - Put them together, we have
        
        $$
        \min_{A\subset V}f'Lf \text{ subject to } Df\perp\vec 1,f'Df=vol(V),f_i=\begin{cases}\sqrt{vol(\bar A)/vol(A)},\text{ if } v_i\in A\\ -\sqrt{vol(A)/vol(\bar A)},\text{ if } v_i\in \bar A\end{cases}
        $$
...
- Leads to the relaxed optimization problem
    ...
    $$
    \min_{f\in\mathbb{R}^n}f'Lf \text{ subject to } Df\perp\vec 1,f'Df=vol(V)
    $$
    
- Now we substitute $$g:=D^{1/2}f$$, we have

$$
\min_{g\in\mathbb{R}^n}g'D^{-1/2}LD^{-1/2}g \text{ subject to } g\perp\vec D^{1/2}1,\| g\| ^2=vol(V)
$$

- Observe that $$D^{-1/2}LD^{-1/2}=L_{sym},D^{1/2}\vec 1$$ is the first eigenvector of $$L_{sym}$$, and $$vol(V)$$ is a constant.
