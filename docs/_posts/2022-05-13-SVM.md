...
- It’s not always plausible to classify all the data points correctly with a hyperplane, so we need to tolerate some error classification which not satisfies the restriction: $$y_i ( w^T \vec x_i + b ) \ge 1$$. We call this soft margin SVM
...
- When $$C=+\infty$$, objective function force all the data points subject to our restriction $$y_i ( w^T \vec x_i + b ) \ge 1$$. Otherwise, the function tolerates some error classification.
